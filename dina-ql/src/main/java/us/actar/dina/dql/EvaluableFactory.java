...
public class EvaluableFactory {
  ...
  public static Value evaluate (Machine machine, Expression expression) {
    return Optional.ofNullable (expression)
      .map (x -> new EvaluableFactory ().create (x))
      .map (e -> e.evaluate (new EvaluationContext (machine, null)))
      .orElse (null);
  }
  ...
  private static class Visitor implements ExpressionVisitor {
    ...
    public void visit (Parenthesis parenthesis) {
      ...
      stack.push (new EvaluableFunction (PrimitiveType.BOOLEAN,
    ...
    public void visit (AndExpression andExpression) {
      combine (andExpression, (left, right) -> {
        left.ensureBoolean ();
        right.ensureBoolean ();
        return new EvaluableFunction (PrimitiveType.BOOLEAN,
    ...
    public void visit (EqualsTo equalsTo) {
      combine (equalsTo, (left, right) -> {
        PrimitiveType.ensureSame (left.getType (), right.getType ());
        return new EvaluableFunction (PrimitiveType.BOOLEAN,
    ...
    public void visit (Addition addition) {
      combine (addition, (left, right) -> {
        left.ensureNumber ();
        right.ensureNumber ();
        return new EvaluableFunction (PrimitiveType.mergeNumbers (left.getType (), right.getType ()),
    ...
    public void visit (NotExpression notExpression) {
      ...
      stack.push (new EvaluableFunction (PrimitiveType.BOOLEAN, c -> new Value (!not.evaluate (c).getBoolean ())));
    }
    ...
    public void visit (net.sf.jsqlparser.schema.Column tableColumn) {
      ...
      Table table = Schema.SCHEMA.getTable (tableName);
      if (table == null)
    ...
    public void visit (SignedExpression signedExpression) {
      ...
      stack.push (new EvaluableFunction (x.getType (), c -> {
        ...
        return x.getType () == PrimitiveType.LONG ? new Value (- v.getLong ()) : new Value (- v.getDouble ());
      }));
    }
